...
export class ElasticsearchService {
     ...
     */
    async initialize(retries = 5, delay = 2000): Promise<void> {
        for (let i = 0; i < retries; i++) {
            try {
                await this.client.ping();
                logger.info('✅ Connected to Elasticsearch');
                await this.createEmailIndex();
                logger.info('✅ Email index is ready');
                return;
            } catch (error) {
                if (i === retries - 1) {
                    logger.error('❌ Failed to initialize Elasticsearch after multiple retries:', error);
                    throw error;
                }
                logger.warn(`⚠️ Could not connect to Elasticsearch. Retrying in ${delay / 1000} seconds... (${i + 1}/${retries})`);
    ...
    private async createEmailIndex(): Promise<void> {
        try {
            ...
            if (!indexExists) {
                ...
                logger.info(`📄 Created email index: ${this.indexName}`);
            } else {
                logger.info(`📄 Email index already exists: ${this.indexName}`);
            }
        } catch (error) {
    ...
    async indexEmail(email: Email): Promise<boolean> {
        try {
            ...
            if (existingEmail) {
                logger.debug(`📧 Email already indexed: ${email.messageId}`);
                return false;
            }
            ...
            logger.debug(`📧 Indexed email: ${email.subject} [${email.messageId}]`);
            return true;
        } catch (error) {
     ...
     */
    async bulkIndexEmails(emails: Email[]): Promise<{ indexed: number; skipped: number }> {
        ...
        try {
            ...
            logger.info(`📧 Bulk indexed ${indexed} emails, skipped ${skipped} duplicates`);
     ...
     */
    async searchEmails(query: EmailSearchQuery): Promise<PaginatedResponse<Email>> {
        try {
            const {
                query: searchQuery,
                account,
    ...
    async getEmailByMessageId(messageId: string): Promise<Email | null> {
        try {
            ...
            if (result.hits.hits.length > 0) {
                return result.hits.hits[0]._source as Email;
            }
     ...
     */
    async updateEmailCategory(id: string, category: EmailCategory): Promise<void> {
        try {
            ...
            if (!exists) {
                logger.warn(`⚠️ Email document not found for category update: ${id}`);
                return;
            }
            ...
            logger.debug(`📧 Updated email category: ${id} -> ${category}`);
        } catch (error) {
     ...
     */
    async getEmailStats(account?: string): Promise<EmailStats> {
        try {
            ...
            if (categoryResult.aggregations?.categories) {
                (categoryResult.aggregations.categories as any).buckets.forEach((bucket: any) => {
                    if (bucket.key in categoryCounts) {
                        categoryCounts[bucket.key as EmailCategory] = bucket.doc_count;
                    }
                });
            }
            ...
            if (categoryResult.aggregations?.accounts) {
                (categoryResult.aggregations.accounts as any).buckets.forEach((bucket: any) => {
            ...
            const recentEmails = categoryResult.aggregations?.recent
                ? (categoryResult.aggregations.recent as any).buckets[0]?.doc_count || 0
                : 0;
    ...
    async deleteEmail(id: string): Promise<void> {
        try {
            ...
            logger.debug(`🗑️ Deleted email: ${id}`);
        } catch (error) {
